...
public class WishListPage extends TestBase {
	...
	public void deleteLastProduct() {
		deleteLinkForLastProduct.click();	
	}
	...
	public WebElement getElementFromTheTable(String ProductName, MyWishList column) {
		...
		int columnIndex = getIndexForColumn(column);//find index of column by this method and store into columnindex varibale
...
//find list of rows in table
	...
	public int getIndexForColumn(MyWishList column) {
		List<WebElement> headers = driver
				.findElements(By.cssSelector("table.table.table-bordered.table-hover thead tr td"));//find all headers in table
		...
		for (WebElement webElement : headers)//iterate over headers 
		{
			...
			//if text of heading matches with column name we pss then find index of that header
				{
